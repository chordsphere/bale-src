...
The four injected docs — docs/CLAUDE.md, docs/TARBALL.md,
docs/DOCS.md, docs/CODE.md — cite one another by section number
...
of the four docs must resolve to a numbered heading in the named doc.
...
- bare internal references ("§11.2" with no doc name) — the doc-name
  prefix is what makes a pointer unambiguous across four files;
...
POINTER = re.compile(
    r"`?(?P<doc>CLAUDE|TARBALL|DOCS|CODE)\.md`?(?:'s)?\s+"
...
class DocCrossReferences(unittest.TestCase):
    ...
    def test_all_global_docs_present(self):
        """The guard reads all four docs — a moved or renamed doc must
    ...
    def test_pointers_are_found_at_all(self):
        """Self-test on the parser: the four docs are known to be
        ...
        self.assertGreater(
            total, 50,
            f"only {total} cross-doc pointers parsed across the four "
