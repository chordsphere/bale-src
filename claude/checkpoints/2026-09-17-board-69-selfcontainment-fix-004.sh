...
echo "checkpoint 69-fix v1: writes to nothing in the tree (tmpdir only)"
...
for suite in test_global_doc_selfcontainment test_response_lint test_schema_embeds; do
...
rm -rf "$tmp"
if [ "$fails" -ne 0 ]; then echo "checkpoint 69-fix v1: $fails probe(s) failed"; exit 1; fi
echo "checkpoint 69-fix v1: all probes passed"; exit 0
