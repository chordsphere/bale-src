#!/usr/bin/env bash
# board-70 blind checkpoint v1 — injected-surface reachability.
...
# P1 — the unnamed-pointer class is gone from every injected
# surface (five docs + both injected tools), wrap-tolerant.
hits=0
for f in docs/CLAUDE.md docs/TARBALL.md docs/DOCS.md docs/CODE.md \
         docs/PLANNER.md tools/craft_response.py \
...
# surfaces: the injected crafter and the installed schema.
