...
2026-09-01 — artifact-borne resolution for tarball-bearing verbs (board 71, v0.4.25). The disambiguation path this ADR deferred to "a sid argument when more than one session is open" is answered differently for a verb that holds a response artifact: `bale retry <tarball>` now resolves its session from the tarball's own `manifest.responds_to`, however many sessions are open — the same registry lookup bare `bale apply` performs — and refuses before touching any HOLD state when that sid is closed (naming the record's last outcome), unknown to the repo, or unreadable from the tarball. `--sid` survives on retry as a vetting flag: absent, resolution is artifact-borne; present and matching, proceed; mismatching, refuse naming both sids and the tarball. The demotion is retry-only; revert, unlock, handoff, and amend-checkpoint hold no artifact and keep the several-open sid requirement. The pattern generalizes to any future verb that receives a manifest-bearing artifact: the artifact names the session, the operator's flag vets it.
