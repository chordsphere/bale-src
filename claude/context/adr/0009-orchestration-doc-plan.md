...
2026-08-16: ladder corrected by the 2026-08-15 chat ruling (recorded at the master desk): step 3's standalone global promotes instead as a *section* of the one conditional-layer planner doc — orchestration doctrine relocated past `docs/PLANNER.md`'s core banner at the extraction session (this session), tombstone left in place; the promotion item is discharged early, and S6 inherits "ratify and churn the orchestration half of PLANNER.md" in its place. Any future physical re-split defers to the injection-model decision, with the banner as the pre-marked seam.
