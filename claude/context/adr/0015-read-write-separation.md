# ADR-0015: Separate the read set from the write forecast; scope is what a session forecasts landing, not what it was shown
...
2026-08-07 — ratified at the master desk (board-13 sitting: E1–E5
all ratified; E3 with the read-side ships-the-oracle refusal
included) and landed Accepted by the Session A implementation
(`2026-08-07-board-13a-forecast-surface-004`, v0.4.1): the `--write`
flag family, the forecast record and `resolved_scope` stamp, the
re-based pack-time gates, and the ADR-0007 status flip. Sessions B
(telemetry epoch, stats rows, apply-side refusal text) and C
(contract-doc propagation per E1) land the remainder of the arc.
