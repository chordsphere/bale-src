# Board 5 — bale stats / trust ledger implementation brief (rev B, 2026-08-01)

Rev B incorporates session A's landed behavior as ratified from its
notes: the clarification stamp is close-keyed (D1), the lineage stamp
is a same-invocation second write (D4), and the clarification
summary's `at` is preserved-file mtime, nullable (D1). Session A has
merged; D8's session A section and D7 are historical record.
...
| # | Question | Decision |
|---|----------|----------|
| D1 | Transient inputs | Promote at the event: bailout attempts embed `diagnostics` verbatim; every closing outcome (applied, reverted, bailout, unlocked — command-agnostic) stamps a bale-computed `clarification` summary read from `.bale/clarifications/<sid>/` at close time. `bale stats` reads `claude/telemetry/` only — never `.bale/` |
...
| D4 | Reverse lineage | Parent's superseded-by-split closure attempt gains additive `superseded_by: <child-sid>`, stamped in place by a second write in the same pack invocation once the child sid mints; envelope untouched. No apply-side appends; no backfill |
...
- **Clarification.** §8.10.2's "a clarification writes no telemetry
  ...
  Instead, every **closing** event stamps an additive `clarification`
  key on its attempt. The rule keys on the closing *outcome*, never
  the command: all four closing outcomes stamp — `applied`,
  `reverted`, `bailout`, and `unlocked` (the last covering `bale
  unlock`, pack's supersession close, the read-only sweep, and
  crash-debris alike, via the shared close path) — while `held`,
  `scope-drift-refused`, `rejected`, and rollback events do not.
  (Ratified from session A's notes; the earlier command-enumerated
  phrasing under-counted pack-driven closes, which would have left
  every pack-closed session outside the clarification epoch
  denominator forever.) The stamp is
  ...
  `at` is the preserved file's mtime (ISO 8601 UTC), nullable for the
  unreadable-stat edge — the manifests carry no timestamp of their
  own; a preservation-time stamp is a queued post-B follow-up.
  The `clarification` key itself is a non-nullable object: the stamp
  is computed, never null — absence is the only "no data" state.
...
writes the parent's closure attempt (`superseded-by-split`, command
`pack`) and, later in the same pack invocation — after
`next_session_id` mints the child sid, which is deliberately late so
aborted packs don't burn NNNs — a second write sets an additive
`superseded_by: "<child-sid>"` key on that closure attempt in place
(`stamp_superseded_by`). The stamp enriches the closure event rather
than adding one, so the envelope (`updated_at`, `outcome`) is
untouched, and an aborted pack's idempotent re-run re-stamps in
place: the completing pack's child wins, one attempt, one key.
(Ratified from session A's notes; rev A's "at the exact moment it
knows the child sid" phrasing didn't survive pack's real control
flow — close resolves before the wizard, the sid mints after.)
Single-writer is preserved: no cross-session write timing,
...
directions), including one record whose `records[].at` is null (the
unreadable-stat edge — the aggregator must tolerate it); a pack-closed
`unlocked` session carrying a `rounds: 0` stamp (the close-keyed rule
puts these inside the epoch denominator); missing work_class →
`unclassed`; post-close
...
(fixture corpus + aggregation suite). Sanctioned rider from session
A's notes: reword `test_undo_appends_re_applied`'s stale rationale
comment in `tests/test_rollback_telemetry.py` — the interleaved
commit it flags as friction is no longer required post-guard-change;
keep the test itself as the committed-variant coverage. Comment-only;
no behavior change.

*(Session A landed and merged 2026-08-01; its section above stands as
