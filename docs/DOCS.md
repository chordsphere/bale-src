...
|----------|----------|-----------|----------|--------|
| **Workflow** | `CLAUDE.md`, `TARBALL.md`, `DOCS.md` | Rarely changed; project-agnostic; always injected by bale into every request | global (in the bale tool's installation) | `ALL_CAPS.md` |
...
`DOCS.md`) — those are injected by bale from its own installation
and are not part of the project's inventory.
...
- **Project-state files** — `ALL_CAPS.md`. Limited set:
  ...
  `DOCS.md` follow the same convention but live in the bale tool,
  not in the project.)
